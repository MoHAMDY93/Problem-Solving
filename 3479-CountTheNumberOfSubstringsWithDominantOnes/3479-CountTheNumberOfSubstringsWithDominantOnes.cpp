// Last updated: 6/10/2026, 7:13:34 PM
class Solution {
public:
    int numberOfSubstrings(string s) {
        ...
        for (int i = 0; i < len; i++) {
            for (int j = i; j < len;) {
                ...
                if (ones >= zeros * zeros) {
                    skip = max((int)sqrt(ones) - zeros, 1); // max, so it doesnt go negative
                    count += min(skip, len - j); // min, so it doenst go above remaining length
                }
